...
- Only push to the `main` branch for now. Do not push to any other remote branch until the user says otherwise.
- Always commit and push to `main` when a change is finished (after the Docker/test verification below passes) — don't leave finished work sitting unpushed in a worktree branch.
